...
class TextCtrlAutoComplete(wx.TextCtrl, listmix.ColumnSorterMixin):
	def __init__(self, parent, colNames=None, choices = None,
		...
		#load the data
		if multiChoices: self.SetMultipleChoices(multiChoices,
				colSearch=colSearch, colFetch=colFetch)
...
class test:
	def __init__(self):
		...
		self.dynamic_choices = [
						...
						'fabulous', 'fantastic', 'friendly', 'forgiving',
						'feature',
		...
		app = wx.PySimpleApp()
		frm = wx.Frame(None,-1,"Test",style=wx.TAB_TRAVERSAL | \
	...
	def onBtMultiChoice(self, event):
		#change the choices
		self._ctrl.SetMultipleChoices([("Test", "Hello"),
			("Other word", "World"), ("Yes!", "it work?")], colFetch=1)
		self._ctrl.SetEntryCallback(None)
		self._ctrl.SetMatchFunction(None)
	...
	def onBtDynamicChoices(self, event):
		'''
		Demonstrate dynamic adjustment of the auto-complete list, based on
	...
	def setDynamicChoices(self):
		...
		choices = [choice for choice in self.dynamic_choices if self.match(text,
			choice)]
